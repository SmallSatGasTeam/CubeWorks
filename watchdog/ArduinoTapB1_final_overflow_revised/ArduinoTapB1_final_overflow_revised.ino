...
const int BOOT_DELAY = 5000; //!!! The Arduino crashes if the BOOT_DELAY variable is more than about 3000. !!!
//!!! The Pi also takes around 50 - 60 seconds to fully boot. 5 seconds is not enough time. !!!
//The pi delay const
#define PI_CHECK 5000
//set LED to high
const int LED = 13;
...
void turn(bool check)
{
  if(check == false)
  {
    ...
    delay(1000);
    Serial.println("1 second delay over");
    //pi on
    digitalWrite(MOSFIT, LOW);
    Serial.println("Pi on");
    //delay to give the pi time to boot, it will have a max of 5 seconds to boot
    //delay(BOOT_DELAY);
    //!!! The Arduino crashes if the BOOT_DELAY variable is more than about 3000. !!!
    long start_time = millis();
    long current_time = millis();
    while(start_time + BOOT_DELAY > current_time){
      current_time = millis();
      Serial.print(start_time);
      Serial.print(" --- ");
      Serial.println(current_time);
    }
    Serial.println("Pi rebooted");
  }
}
